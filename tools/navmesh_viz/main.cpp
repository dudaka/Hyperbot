...
// center, R=1 -> 3x3, R=2 -> 5x5), clamped to valid sector bytes.
...
int main(int argc, char **argv) {
  ...
  std::cout << "Loading navmesh from \"" << dataPath.string()
            << "\" (center=0x5c87, ring radius=" << radius << ")\n";
  // Load every region up to the requested radius; in serve mode the client can
  // then request any smaller ring without a reload.
  std::vector<std::set<uint16_t>> regionSetsByRadius;
  for (int r = 0; r <= radius; ++r) {
    regionSetsByRadius.push_back(clusterRegionIds(kCenterRegionId, r));
  }
  const std::set<uint16_t> clusterIds = clusterRegionIds(kCenterRegionId, radius);
  sro::pk2::Pk2ReaderModern reader{dataPath};
  sro::pk2::NavmeshParser parser{reader};
  parser.setRegionAllowList(clusterIds);
  ...
  if (mode == "serve") {
    ...
    navmesh_viz::runServer(navmesh, triangulation, regionSetsByRadius, port);
    return 0;
  }

  // dump mode: write geometry to a file and run a sample path query.
  const std::string json =
      navmesh_viz::buildGeometryJson(navmesh, triangulation, clusterIds);
